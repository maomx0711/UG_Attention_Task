...
### 模型5: 贝叶斯效用模型 (Bayesian Utility Model) - 测试H2++

正式的贝叶斯框架，使用幂函数效用和适当的先验分布：

```
V(r) = u(r) - v(c(r))
u(r) = r^α                    # 幂函数奖励效用
v(c) = λ × c^β                # 幂函数惩罚负效用
c(r) = c₀ × exp(-γ × r)       # 惩罚随金额减少

P(accept) = Φ((V(r) - θ) / σ)  # 逻辑函数
```

**先验分布:**
- α ~ Lognormal(0, 0.5): 奖励敏感度 (α<1: 风险规避, α>1: 风险寻求)
- λ ~ Lognormal(0, 0.5): 损失规避系数 (λ>1: 损失规避)
- β ~ Gamma(2, 1): 惩罚敏感度
- γ ~ Gamma(2, 1): 惩罚衰减率
- θ ~ Normal(0, 2): 决策阈值
- σ ~ HalfNormal(1): 决策噪声
...
**心理学解释:**
- **风险态度**: α < 1 表示边际效用递减（风险规避）
- **损失规避**: λ > 1 表示对惩罚比奖励更敏感

### 模型6: 隐马尔可夫模型 (Hidden Markov Model) - 测试H3
